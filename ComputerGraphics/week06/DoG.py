...
def DoG_pyramids(src,g=5,l=4):
    ...
    for s in range(4):
        for j in range(g):
            ...
            h,w = scales[s]
            gaussian = cv2.resize(gaussian,dsize=(w,h),interpolation=cv2.INTER_LINEAR)
            gaussian_pyramids[s][:,:,j] = gaussian
    print("calc Difference of Gaussian")
    DoG = [
    ...
    for s in range(4):
        for i in range(l):
            prev = gaussian_pyramids[s][:,:,i+1]
            next = gaussian_pyramids[s][:,:,i]
            DoG[s][:,:,i] = prev - next
